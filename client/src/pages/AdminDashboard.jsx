...
const AdminDashboard = () => {
  ...
  const handleAddTrain = (e) => {
    ...
    sendTrainUpdate(
      `New train added: ${train.name} (${train.number}) at Platform ${train.platform}`
    );
    setNewTrain({
      name: "",
      number: "",
  ...
  const handleDeleteTrain = (id) => {
    ...
    if (train) {
      ...
      sendTrainUpdate(
        `Train ${train.name} (${train.number}) has been cancelled`
      );
    }
  };
  ...
  const handleStatusChange = (id, newStatus) => {
    const updatedTrains = trains.map((train) => {
      if (train.id === id) {
        ...
        sendTrainUpdate(
          `${train.name} (${train.number}) status updated to: ${newStatus}`
        );
        return updatedTrain;
      }
      return train;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-600 dark:text-gray-300">
            Manage station operations and real-time updates
          </p>
        </motion.div>
              ...
              <form onSubmit={handleAddTrain} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-gray-200"
                    required
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewTrain({ ...newTrain, number: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-gray-200"
                    required
                  />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewTrain({ ...newTrain, platform: e.target.value })
                      }
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-gray-200"
                      required
                    />
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        value={train.status}
                        onChange={(e) =>
                          handleStatusChange(train.id, e.target.value)
                        }
          ...
          >
            ...
            <div className="space-y-4">
              <button
                onClick={() =>
                  sendTrainUpdate(
                    "Attention passengers: All trains are running on time today."
                  )
                }
              ...
              >
                ...
                <p className="text-sm text-blue-700">
                  Send a general announcement about train operations
                </p>
              </button>
              <button
                onClick={() =>
                  sendPlatformUpdate(
                    "Platform 2 is temporarily closed for maintenance. Passengers are requested to use alternative platforms."
                  )
                }
              ...
              >
                ...
                <p className="text-sm text-orange-700">
                  Announce platform maintenance or closures
                </p>
              </button>
              <button
                onClick={() =>
                  sendTrainUpdate(
                    "Due to weather conditions, some trains may experience delays. Please check with station staff for updates."
                  )
                }
              ...
              >
                ...
                <p className="text-sm text-yellow-700">
                  Send weather-related announcements
                </p>
              </button>
            </div>
          </motion.div>
